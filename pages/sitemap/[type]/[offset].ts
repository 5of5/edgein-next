...
const getDataForType = async (type: string, offset: number) => {
  try {
    ...
    if (offset > maxOffset) {
      throw new Error(
        `Invalid offset: ${offset}. Maximum allowed offset is ${maxOffset}`,
      );
    }
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log(
        `[Sitemap] ${type}: ${startIndex + 1} to ${Math.min(
          startIndex + URLS_PER_SITEMAP,
          totalCount,
        )} of ${totalCount}`,
      );
    }
...
const generateSitemapXML = (items: any[], type: string) => {
  ...
  const getSlug = (item: any) =>
    type === 'lists' || type === 'groups' ? item.id : item.slug;
...
export const getServerSideProps: GetServerSideProps = async ({
  ...
  try {
    ...
    if (
      !QUERIES[type as keyof typeof QUERIES] ||
      isNaN(offsetNum) ||
      offsetNum < 0
    ) {
      return { notFound: true };
