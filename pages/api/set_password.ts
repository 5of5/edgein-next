...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (!user.auth0_user_pass_id) return res.status(404).send('Invalid request for password');
  ...
  try {
    ...
    if (!managementTokenRequest.ok) {
      ...
      return res.status(managementTokenRequest.status).send(managemetTokenErrResponse.error_description)
    }
    ...
    if (!setPasswordRequest.ok) {
      const setPasswordErrResponse = JSON.parse(await setPasswordRequest.text());
      return res.status(setPasswordRequest.status).send(setPasswordErrResponse.message)
    }
