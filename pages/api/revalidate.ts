...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    const path = typeof req.query.page === 'string' ? req.query.page : req.query.page[0]
    await res.unstable_revalidate(`${path}?revalidation_auth=${process.env.REVALIDATION_AUTH_TOKEN}`)
