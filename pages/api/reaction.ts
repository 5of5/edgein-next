...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  const { sentiment, revalidatePath } = await updateResourceSentimentCount(resourceType, resourceId, token, sentimentType ? sentimentType : req.body.listName, Boolean(follow), Boolean(existsFollows))
  // if (revalidatePath) {
