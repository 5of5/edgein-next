...
const ElemNotificationPopover: FC<Props> = ({
  ...
  return (
    <Popover
      ...
      style={popoverStyle}>
      <Popover.Button className="inline-flex items-center text-sm rounded-full aspect-square p-1 transition ease-in-out duration-150 group bg-black ring-inset ring-1 ring-slate-200 hover:text-primary-500 hover:bg-slate-200 focus:outline-none focus:ring-1">
        <IconEllipsisHorizontal
      ...
      <Transition
        ...
        <Popover.Panel className="absolute right-0 overflow-hidden w-48 p-1 divide-y divide-slate-100 rounded-lg bg-black shadow-lg ring-1 ring-black/5">
          {({ close }) => (
            <>
              {!notification.read && (
                <button
