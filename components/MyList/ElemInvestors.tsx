...
export const ElemInvestors: FC<Props> = ({
			...
			if (selected.length > 0 && resourceList?.length === selected.length) {
				...
				setSelected((prev) => {
					...
					resourceList?.forEach(({ id }) => {
						if (!items.includes(id as number)) items.push(id as number);
					});
					return items;
				});
			}
	...
	return (
		<div className="rounded-lg p-3 bg-white col-span-3 mt-10 mb-10 ">
			<h2 className="font-bold text-dark-500 text-xl capitalize">
				...
				<table className="w-full">
					...
					<tbody>
						...
						{(!resourceList || resourceList?.length === 0) && (
							<tr>
								<td colSpan={4} className="text-center px-1 py-2">
									No Investors
								</td>
							</tr>
						)}
