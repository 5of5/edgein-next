import React, { MutableRefObject, useRef, useEffect, useState, Fragment } from "react";
          ...
          <div className="min-w-0 flex-1">
            <div>
              ...
              <div className="text-sm text-slate-600">
                <Link href={`/people/${noteAuthor?.slug}`} passHref>
                  <a>{noteAuthor?.name}</a>
                </Link>
                <span aria-hidden="true"> · </span>
                <ElemTooltip
                  content={`${moment(data?.created_at).format("LL h:mma")}`}
                  size="md"
            ...
            <div key={comment.id} className="flex items-center gap-2">
              ...
              {comment.created_by_user_id === user?.id && (
                <Popover className="relative">
                  ...
                  >
                    <Popover.Panel className="absolute left-1/2 -translate-x-1/2 bg-white w-48 z-10 rounded-md shadow-md p-2">
                      <button
                        onClick={() => {
