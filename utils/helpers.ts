...
import {
  Companies_Bool_Exp, Vc_Firms_Bool_Exp,
} from "@/graphql/types";
...
export const processInvestorsFilters = (
  ...
  if (selectedFilters?.numOfInvestments?.maxVal) {
  	filters._and?.push({
  		_and: [
  			{ num_of_investments: { _gt: selectedFilters?.numOfInvestments?.minVal ?? 0 } },
  			{ num_of_investments: { _lte:selectedFilters?.numOfInvestments?.maxVal } },
  		],
  	});
  }
