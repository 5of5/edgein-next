...
export const verify = (ip: string): boolean => {
    ...
    const userTriggerFetchersPrefixes = (userTriggerFetchers as Googlebot).prefixes;
    const allPrefixes = [...specialCrawlersPrefixes, ...googlebotPrefixes, ...userTriggerFetchersPrefixes];
    const ipv4Prefixes = allPrefixes.filter((prefix) => 'ipv4Prefix' in prefix).map((prefix) => (prefix as Ipv4Prefix).ipv4Prefix);
    const ipv6Prefixes = allPrefixes.filter((prefix) => 'ipv6Prefix' in prefix).map((prefix) => (prefix as Ipv6Prefix).ipv6Prefix);
    return ipv4Prefixes.some(ipv4 => matches(ip, ipv4)) || ipv6Prefixes.some(ipv6 => matches(ip, ipv6));
}