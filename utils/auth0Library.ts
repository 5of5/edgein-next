...
async function getUserInfoFromToken(accessToken: string) {
  ...
  const userInfoFetchRequest = await fetch(`${process.env.NEXT_PUBLIC_AUTH0_ISSUER_BASE_URL}/userinfo`, {
    method: 'GET',
...
async function linkTwoAccount(primaryId: string, secondayId: string, secondayProvider: string) {
  ...
  if (!managementTokenRequest.ok) {
    const managemetTokenErrResponse = JSON.parse(await managementTokenRequest.text());
  ...
  const setPasswordObject = JSON.stringify({ provider: secondayProvider, user_id: secondayId });
  const linkUserAccountsRequest = await fetch(`${process.env.AUTH0_ISSUER_BASE_URL}/api/v2/users/${primaryId}/identities`, {
    method: 'POST',
    headers: myHeaders,
    body: setPasswordObject,
  });
  if (!linkUserAccountsRequest.ok) {
    const linkUserAccountsErrResponse = JSON.parse(await linkUserAccountsRequest.text());
    throw linkUserAccountsErrResponse.message;
  }
}

export default { getUserInfoFromToken, linkTwoAccount }